...
// Desktop-only: the /expenses page renders a data table on desktop but mobile
...
const expectedStatusBadge = (expense: Expense): string | null => {
  if (expense.excludeFromCalcs) {
    return 'Excluded';
  }

  if (expense.endDate && daysUntil(expense.endDate) < 0) {
    return 'Ended';
  }
...
test('expenses list renders seeded rows, amounts, status and frequency', async ({
  page,
}, testInfo) => {
  test.skip(
    isMobileProject(testInfo),
    'Expense list table is desktop-only; mobile uses the card grid',
  );
  ...
  const expenses = (await expensesResponse.json()) as Expense[];
  expect(expenses.length).toBeGreaterThan(0);

  // Pick a stable target row: the first non-excluded expense.
  const target =
