...
import ExpensesOverview from '@/features/dashboard/components/ExpensesOverview';
...
function createExpensesQuery(expenses: Expense[], isLoading = false) {
  return {
...
function renderOverview() {
  ...
  return render(
    <QueryClientProvider client={queryClient}>
      <ExpensesOverview isOpen onOpenChange={vi.fn()} />
    </QueryClientProvider>,
  );
}
...
describe('ExpensesOverview', () => {
  ...
  test('renders the section heading and the next due filter', () => {
    vi.mocked(useApiGetAllExpenses).mockReturnValue(createExpensesQuery([]));

    renderOverview();

    expect(
      screen.getByRole('heading', { name: 'Expenses Overview' }),
    ).toBeInTheDocument();
  ...
  test('renders expenses due within the stored period and omits later expenses', () => {
    ...
    vi.mocked(useApiGetAllExpenses).mockReturnValue(
      createExpensesQuery([dueSoon, dueLater, dueMuchLater]),
    );
  ...
  test('excludes expenses marked as excluded from calculations', () => {
    const included = createExpense({
      rowId: 'expense-1',
      description: 'Included expense',
      nextDue: dateIsoFormat(addDays(localToday(), 3)),
    });

    const excluded = createExpense({
      rowId: 'expense-2',
    ...
    vi.mocked(useApiGetAllExpenses).mockReturnValue(
      createExpensesQuery([included, excluded]),
    );

    renderOverview();

    expect(screen.getByText('Included expense')).toBeInTheDocument();
    expect(screen.queryByText('Excluded expense')).not.toBeInTheDocument();
  });

  test('filters expenses using the stored 7 day period', () => {
    givenStoredPeriod(7);
    ...
    const dueInTenDays = createExpense({
      rowId: 'expense-2',
    ...
    vi.mocked(useApiGetAllExpenses).mockReturnValue(
      createExpensesQuery([dueSoon, dueInTenDays]),
    );
  ...
  test('persists the selected period and re-filters the expenses', async () => {
    ...
    vi.mocked(useApiGetAllExpenses).mockReturnValue(
      createExpensesQuery([dueSoon, dueLater]),
    );
    ...
    expect(setDashboardDataMock).toHaveBeenCalledWith({ expensesPeriod: 30 });
    expect(screen.getByText('Due in twenty days')).toBeInTheDocument();
  });

  test('shows card skeletons while the expenses are loading', () => {
    const dueSoon = createExpense({
      rowId: 'expense-1',
      description: 'Due in three days',
      nextDue: dateIsoFormat(addDays(localToday(), 3)),
    ...
    vi.mocked(useApiGetAllExpenses).mockReturnValue(
      createExpensesQuery([dueSoon], true),
    );
  ...
  test('reports API failures through the error context', () => {
    const failure = new FailResult(new UnexpectedError('Expenses failed'));
