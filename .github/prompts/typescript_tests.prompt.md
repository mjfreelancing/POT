---
name: typescript_tests
description: Create or update TypeScript/JavaScript tests with deterministic setup and behavior-focused assertions.
---

Create tests for TypeScript/JavaScript code.

## Scope

- Place tests in the nearest test folder.
- Keep tests deterministic and assert behavior over implementation details.

### Web client

- Target client tests and supporting test utilities for the active web client.
- Place tests in the nearest client test location or mirrored test folder.
- Prefer user-observable outcomes over implementation details.

## Workflow

1. Place tests in the nearest test folder.
2. Follow scoped instruction files for framework/test conventions.
3. Keep tests deterministic and assert behavior over implementation details.
4. Run narrow scope first, then broaden only if required.

## Execution

Adjust the example paths below to match your repository structure.

- `npm run test` (from `.\Source\Client\my-app`)
- Broaden only when necessary, such as UI or watch modes documented by the repository.

## Expansion Notes

- Keep framework-specific TypeScript and client test conventions in instruction files.
- Add repository-specific commands and paths under `Execution`.
  - POT: Client tests live under `Source/Client/pot-react/tests/**`.
  - POT: Mirror tests into the repository's `tests/**` location.
  - POT: Run `npm run test` from `Source/Client/pot-react`.
  - POT: Use `npm run test:ui` when interactive UI mode is needed.
