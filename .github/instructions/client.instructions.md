...
- Do not edit shadcn primitives in `src/components/ui` unless explicitly requested.
...
- Keep error normalization centralized in `src/api/interceptors/axiosInterceptors.ts`; extend typed errors in `src/api/errors/*`.
...
- Keep exports at the bottom of files and remove unused imports.
...
- For toasts, keep copy concise, do not stack duplicate event toasts, and do not pass a custom icon to `ErrorToast`.
...
- If a client pattern is repeated in 3+ places, update the relevant client instruction files in the same change.
