...
**Status:** In Progress — execution underway (Increments 0–4 complete & validated)
**Priority:** Medium
...
| 5   | Utilities — date-fns 4, lucide-react 1.40                                                                        | ⏳ Next        | —                                                                      |
| 6   | zod 4                                                                                                            | ⛔ Not started | —                                                                      |
| 7   | recharts 3                                                                                                       | ⛔ Not started | —                                                                      |
| 8   | react-day-picker 10                                                                                              | ⛔ Not started | —                                                                      |
...
4. Coverage-audit pre-work for the risky increments (6–9): add guard tests first per §13 (zod form schemas, ProjectionChart smoke, real `ui/calendar` render, real `DataTable` render).
...
- **Inc 5 — Utilities refresh (low–med).** date-fns 4.4.0 + lucide-react 1.40.0 (rename deprecated `-Icon` aliases via codemod + manual for shadcn `ui/*`); verify the ARIA default on icons doesn't alter labels we set.
...
- **Inc 8 — react-day-picker 10.0.1 (med–high).** Update `calendar.tsx` classNames/components to v10 keys + package decision (DQ-2: `@daypicker/react` vs compat alias); port `EnrichedCalendar`/`EnrichedDatePicker`; update VisualBehavior + ARIA assertions.
...
| 5 date-fns 4 + lucide 1.40 | `lib/dateUtils` 100%, `lib/chartUtils` 100%, icon-bearing components + `tsc` catches renamed exports                                                                                                                                                                                                                                     | ✅ Adequate                                                | none                                                                                                                                                               |
