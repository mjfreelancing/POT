...
**Status:** In Progress — Inc 0–4 committed; Inc 5–8 validated; Inc 8b (shadcn v4 ui layer) validated
**Priority:** Medium
...
3. Open decisions: DQ-1 (TS 6 first, then 7), DQ-2 (**resolved** — `react-day-picker` is the real package; `@daypicker/react` is the compat alias), DQ-3 (coverage — measured: no re-baseline needed), DQ-5 (run all 13 increments vs stop after safe set), DQ-7 (no React Compiler opt-in).
4. Coverage-audit pre-work for the risky increments (6–9): zod form-schema guard tests **done** (Inc 6), chart smoke tests **done** (Inc 7), real `ui/calendar` render test **done** (Inc 8); remaining pre-work — real `DataTable` render (Inc 9).
