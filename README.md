# POT - Paid On Time

<img src="AppLogo.png" alt="POT Logo" style="width:200px;"/>

**A financial management application to help you track accounts, expenses, and income — ensuring your bills are paid on time.**

## About POT

POT is a modern web application designed to help you manage your finances by projecting your financial status based on:

- Current bank account balances
- Scheduled income (credits)
- Planned expenses (debits)

With interactive visualizations and projections, POT provides a clear picture of your financial future, helping you make informed decisions and ensure all debts are paid on time.

### Key Features

- **Financial Projections** - Visualize your account balances months in advance
- **Expense Tracking** - Manage recurring and one-time expenses
- **Income Management** - Track multiple income sources and payment schedules
- **Account Management** - Monitor balances across all your financial accounts
- **Data Import/Export** - Backup and restore your financial data

---

## Documentation

For detailed information, see:

- [Getting Started Guide](Docs/GETTING-STARTED.md) - Setup and installation
- [Features Documentation](Docs/Features/overview.md) - Detailed feature descriptions
- [Architecture Overview](Docs/ARCHITECTURE.md) - Technical architecture
- [Development Guide](Docs/Development/) - For developers

---

## License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.
