...
<#
.SYNOPSIS
    Starts the POT E2E PostgreSQL container.

.DESCRIPTION
    Pulls the postgres:13 image if not already present, then starts the
    pot-e2e container group defined in docker-compose.yml.

    The container exposes PostgreSQL on localhost:5432 for use during
    E2E testing.
...
Write-Host 'Starting POT E2E PostgreSQL container...' -ForegroundColor Cyan
...
Write-Host ''
Write-Host 'POT E2E PostgreSQL is running.' -ForegroundColor Green
